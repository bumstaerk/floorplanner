    ...
    const applyAngleSnap = useCallback(
        (start: Point2D, end: Point2D): Point2D => {
            ...
            if (snap.enabled && snap.gridSize > 0) {
                snappedEnd.x =
                    Math.round(snappedEnd.x / snap.gridSize) * snap.gridSize;
                snappedEnd.y =
                    Math.round(snappedEnd.y / snap.gridSize) * snap.gridSize;
            }
            ...
            if (drawingFromCornerId) {
                ...
                if (!targetCornerId) {
                    ...
                    if (wallHit) {
                        ...
                        const drawingFromWalls =
                            getWallsAtCorner(drawingFromCornerId);
...
function EditorGrid() {
    ...
    return (
        <group position={[0, -0.005, 0]}>
            <gridHelper
                args={[gridSize, gridDivisions, "#334155", "#1e293b"]}
            />
        </group>
    );
}
