...
function Door2D({
  ...
  const gapColor = colors.openingGap;
...
function Window2D({
  ...
  const lines = useMemo(() => {
    const { dirX, dirY, normX, normY, halfT, startWorld, endWorld } = data;
  ...
  const colors = useThemeColors();
  const color = isSelected ? colors.openingSelected : colors.openingDefault;
  const paneColor = isSelected
    ? colors.openingPaneSelected
    : colors.openingPaneDefault;
  const gapColor = colors.openingGap;
...
function Hole2D({
  ...
  const { dashedEdges } = useMemo(() => {
    const { normX, normY, halfT, startWorld, endWorld } = data;

    // Dashed lines along the left and right face through the gap
    const leftDash: [number, number, number][] = [
      [startWorld.x + normX * halfT, Y + 0.002, startWorld.y + normY * halfT],
      [endWorld.x + normX * halfT, Y + 0.002, endWorld.y + normY * halfT],
    ];
  ...
  const colors = useThemeColors();
  const color = isSelected ? colors.openingSelected : colors.openingPaneDefault;
  const gapColor = colors.openingGap;
