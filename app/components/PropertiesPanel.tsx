    ...
    return (
        <div className="bg-gray-700/30 rounded-lg p-2.5 space-y-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
                ...
                {opening.type === "door" && (
                    <>
                        <label className="text-[10px] text-gray-500">Hinge:</label>
                        <div className="flex rounded overflow-hidden border border-gray-600">
                            <button
                                onClick={() => onUpdate({ hinge: "start" })}
                                className={`text-[10px] px-2 py-0.5 transition-colors cursor-pointer ${
                                    (opening.hinge ?? "start") === "start"
                                        ? "bg-blue-600 text-white"
                                        : "bg-gray-700 text-gray-400 hover:bg-gray-600"
                                }`}
                                title="Hinge at start edge"
                            >
                                ← Start
                            </button>
                            <button
                                onClick={() => onUpdate({ hinge: "end" })}
                                className={`text-[10px] px-2 py-0.5 transition-colors cursor-pointer ${
                                    opening.hinge === "end"
                                        ? "bg-blue-600 text-white"
                                        : "bg-gray-700 text-gray-400 hover:bg-gray-600"
                                }`}
                                title="Hinge at end edge"
                            >
                                End →
                            </button>
                        </div>
    ...
    return (
        <div className="divide-y divide-gray-700/50">
            ...
            <div className="px-4 py-3 flex items-start justify-between gap-2">
                ...
                <button
                    onClick={() => updateFloorplan({ visible: floorplan.visible === false ? true : false })}
                    title={floorplan.visible === false ? "Show floorplan" : "Hide floorplan"}
                    className="flex-shrink-0 mt-0.5 p-1 rounded text-gray-400 hover:text-white hover:bg-gray-700 transition-colors cursor-pointer"
                >
