...
export function StatusBar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-10 flex items-center justify-between px-4 py-2 pointer-events-none">
      {/* Left: drawing hint */}
      <div className="text-xs text-gray-600 dark:text-gray-400 bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm rounded px-3 py-1.5 shadow border border-gray-200/50 dark:border-transparent">
        {mode === "build" && activeTool === "wall" && !drawingFromCornerId && (
          <span>Click to place the first corner of a wall</span>
        )}
        {mode === "build" && activeTool === "wall" && drawingFromCornerId && (
          <span>
      ...
      {/* Right: stats */}
      <div className="text-xs text-gray-500 dark:text-gray-500 bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm rounded px-3 py-1.5 shadow border border-gray-200/50 dark:border-transparent">
        {floors.find((f) => f.id === currentFloorId)?.name ?? "Floor"} •{" "}
